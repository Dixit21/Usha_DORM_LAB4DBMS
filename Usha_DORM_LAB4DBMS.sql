...
select C.CUS_GENDER ,COUNT(C.CUS_GENDER),sum(O.ORD_AMOUNT)
FROM customer as C INNER JOIN  `order` AS O 
ON C.CUS_ID =O.CUS_ID
AND O.ORD_AMOUNT >=3000 
GROUP BY C.CUS_GENDER;
-- 4.Display all the orders along with product name ordered by a customer having Customer_Id=2
SELECT * FROM (SELECT product.PRO_NAME,product.PRO_ID,supplier_pricing.PRICING_ID FROM supplier_pricing 
...
SELECT   S.SUPP_ID,S.SUPP_NAME,  COUNT(S.SUPP_ID),SP.PRO_ID 
FROM supplier as S 
INNER JOIN supplier_pricing AS SP 
ON S.SUPP_ID = SP.SUPP_ID GROUP BY S.SUPP_ID HAVING COUNT(S.SUPP_ID)>1;

-- 6)	Find the least expensive product from each category and print the table with category id, name, product name and price of the product
SELECT C.CAT_ID, sup_pro.minSP, sup_pro.PRO_ID,sup_pro.PRO_NAME, sup_pro.CAT_ID 
...
SELECT Supp_Pro.PRO_ID, Supp_Pro.PRO_NAME, O.ORD_ID, O.ORD_DATE
...
-- END
-- The above code is of stored procedure if we created then we directly call it by
 CALL ServiceRating(); 
-- Although i write the code in one file 